/* 
    1. Write a fat arrow async function that does the following:
        - uses await to fetch data from this endpoint - 'https://swapi.dev/api/people/'
        - uses await to jsonify the response received from the fetch
        - console.log's the results of the fetch before console.logging a message saying 'this should print last'
*/

//NOTE: Write your code below and push back to your github branch.  SUBMIT YOUR GITHUB URL IN CANVAS

const characterList = document.querySelector('list');
const character = document.querySelector('ul');

async function getCharacter(){
    await fetch('https://swapi.dev/api/people/')
    .then (res => res.json())
    .then (json => {
        console.log(json)
        // showCharacter(json)
    })
    console.log('this should print last')

}
getCharacter();

// function showCharacter(slot) {
//     slot.forEach(ul => {
//         let person = document.createElement('li')
//         person.innerText = ul.name
//     })
// }